...
void csr_w(int csr, int32_t val){
    switch(csr) {
        case 0x105:
...
make_EHelper(system){
  ...
  switch(instr.funct3){
    ...
    case 0b001:     // csrrw
        s0 = csr_r(instr.csr);
        csr_w(instr.csr, id_src->val);
        rtl_sr(id_dest->reg, &s0, 4);
        print_asm_template3(csrrw);
        break;
    case 0b010:     // csrrs
        s0 = csr_r(instr.csr);
        csr_w(instr.csr, s0 | id_src->val);
        rtl_sr(id_dest->reg, &s0, 4);
    ...
    case 0b101:// csrrwi
        s0 = csr_r(instr.csr);
        csr_w(instr.csr, id_src->reg);
        rtl_sr(id_dest->reg, &s0, 4);
        print_asm_template3(csrrwi);
        break;
    case 0b110:// csrrsi
        s0 = csr_r(instr.csr);
        csr_w(instr.csr, s0 | id_src->reg);
        rtl_sr(id_dest->reg, &s0, 4);
